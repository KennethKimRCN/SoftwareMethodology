...
public class PhotoControl {
	...
	public void start(Stage primaryStage, Stage previousStage, AlbumUI currUI, Album currAlbum) throws IOException {
		this.primaryStage = primaryStage;
		this.previousStage = previousStage;

		this.currUI = currUI;
		this.userName = this.currUI.getUsername();
	...
	private void updatePhotoPane(Photo newPhoto) {
		...
		this.photoPaneName.setText(newPhoto.getCaption());
	}
	...
	private StackPane getFormattedPhoto(Photo newPhoto) {
		...
		Text text = new Text(newPhoto.getCaption());
	...
	void openPhotoAction(ActionEvent event) throws IOException {
		...
		DisplayPhoto Controller = loader.getController();
	...
	void deletePhotoAction(ActionEvent event) {
		...
		if(result.get() == ButtonType.OK) {
			...
			this.photoFormattedContent.remove(getPhotoIndex(this.photoFormattedContent, this.activePhoto.getCaption()));
	...
	void renamePhotoAction(ActionEvent event) throws IOException {
		...
		String oldCaption = this.activePhoto.getCaption();
		RenamePhoto Controller = loader.getController();
		...
		for(Node node: this.photoFormattedContent.get(photoIndex).getChildren()) {
			if(node instanceof Text && ((Text)node).getText().equals(oldCaption)) {
				((Text)node).setText(this.activePhoto.getCaption());
			}
		}
		//update flow pane
		this.photosContainer.getChildren().clear();
	...
	void copyPhotoAction(ActionEvent event) throws IOException {
		...
		OpenAlbum Controller = loader.getController();
		Controller.start(newStage, this.currUI, this.currAlbum);
	...
	void goBackAction(ActionEvent event) throws IOException {
		...
		Controller.start(this.primaryStage, this.previousStage, this.currUI);
